...
with raw_source as (
    ...
    {% if is_incremental() %}
    where cast(TransactionDate as timestamp) > (
...
deduped as (
    select *,
        row_number() over (
            partition by TransactionID order by TransactionDate desc
        ) as _rn
    from raw_source
),
...
cleaned as (
    ...
    from deduped
    where _rn = 1 and TransactionID is not null and TransactionAmount > 0
)
